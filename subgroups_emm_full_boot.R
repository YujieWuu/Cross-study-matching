...
# X1,2 are effect modifiers, but effects are the same across studies
rm(list=ls())
library(mvtnorm)
library(ggplot2)
library(MatchIt)
...
beta_X_fix     <- matrix(NA, nrow = K, ncol = d) # coef to X for Y
beta_X_fix[1,] <- rnorm(d, 0.1 , 0.1)
beta_X_fix[2,] <- beta_X_fix[1,]

beta_X_fix[3,] <- rnorm(d, 0.6 , 0.1)
beta_X_fix[4,] <- beta_X_fix[3,]

beta_Tx_fix   <- c(1,1,1.5,1.5)
true_cluster  <- c(1,1,2,2)
...
simfun <- function(d = 4, K = 4, size = 4000, boot_num=1000,
                   var_X = 2, alpha = alpha_fix ,
                   beta_X = beta_X_fix , beta_Tx = beta_Tx_fix,
                   match_method = "nearest", 
                   replacement = TRUE){
  
  X          <- rmvnorm(size, mean = rep(1, d), sigma = diag(var_X, d))
  ...
  df_all   <- data.frame(X, Tx, stud)
  df_all   <- df_all[order(df_all$stud),]
  Y        <- c()
  for(i in 1:K){
    X_temp   <- as.matrix(df_all[df_all$stud==i,1:d])
    Tx_temp  <- df_all[df_all$stud==i,"Tx"]
    Y_temp   <- 1 + X_temp%*%beta_X[i,] + Tx_temp*beta_Tx[i] + 0.5*Tx_temp*X_temp[,1] + rnorm(nrow(X_temp),0,1)
    Y        <- append(Y, Y_temp)
  }
  ...
  for(i in 1:K){
    ...
    fit1 <- lm(Y ~ Tx + Tx*X1 + X1 + X2 + X3 + X4, df_temp)
  ...
  for(index_target in 1:K){
    for(index_source in 1:K){
      if(index_target!=index_source){
        ...
        fit1 <- lm(Y ~ Tx + Tx*X1 + X1 + X2 + X3 + X4,
        ...
        fit2 <- lm(Y ~ Tx + Tx*X1 + X1 + X2 + X3 + X4,
        ...
        match_data_treat  <- match.data(match_tx)
        fit3 <- lm(Y ~ Tx + Tx*X1 + X1 + X2 + X3 + X4,
